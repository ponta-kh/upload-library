...
アップロードされたCSVファイルの値に対してバリデーションを行うためのライブラリです。

## 概要

本ライブラリは、UTF-8形式のCSVファイルを解析し、ファイルの値と指定したバリデーション結果を格納した２次元配列を返す関数を提供します。
`papaparse` を利用してCSVをパースし、柔軟なバリデーションルールに基づいて各行・各セルを検証できます。

Reactユーザーはバリデーション関数の戻り値とCSV列情報をpropsに受け取る結果表示用のコンポーネントも利用できます。
本コンポーネントは **shadcn/ui をベース** に作成されており、一部で **lucide-react** を利用しています。
利用する場合は、利用者のプロジェクトに **Tailwind CSS と lucide-react の導入をお願い致します。**
